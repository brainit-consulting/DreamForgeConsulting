...
export const helpContent: Record<string, HelpSection> = {
  ...
  adminGuide: {
    ...
    tips: [
      ...
      "HOW TO CONFIGURE SETTINGS: Settings has 4 collapsible sections — click headers to expand. Athena: system prompt, temperature, max tokens, model list, OpenAI fallback toggle. Email: logo (with size slider), company name, sign-off, tagline, greeting toggles, auto-approval email toggle. Backups: manual backup, restore, retention policy (daily/weekly/monthly counts), backup list with download/restore. Support Plan Defaults: monthly rate, included hours, overage rate, annual free months.",
      ...
      "OWNERSHIP & LICENSING: DreamForge retains full ownership of all source code, architecture, and IP. Clients receive a perpetual, non-exclusive use license maintained through an active support retainer. If the retainer lapses beyond 60 days, we reserve the right to suspend/disable the software. Clients may NOT resell, redistribute, reverse-engineer, or clone the product. Clients may submit a written buyout offer at any time. Custom assets (logos, content) remain client property. These terms are included in every proposal and acknowledged at client approval.",
