...
export const helpContent: Record<string, HelpSection> = {
  ...
  leads: {
    ...
    tips: [
      ...
      "Click the lead name to open the Edit dialog directly",
      ...
      "Green mail icon next to a lead name = outreach email sent (hover for date); amber mail icon = draft pending; no icon = never contacted",
      "Select leads with the checkbox column for bulk outreach — a floating action bar appears at the bottom with lead count and a Continue button",
      ...
      "The Card checkbox (second column) tracks whether you've sent a handwritten postcard to this lead — check it to mark as sent",
    ],
  },
  clients: {
    ...
    tips: [
      ...
      "Trash icon archives the client (soft-delete) — their invoices and financial history are preserved for your records; the client disappears from the active list but data is never lost",
  ...
  invoices: {
    ...
    tips: [
      ...
      "Support invoices are auto-generated as DRAFT on the 1st (one per plan per month — duplicates are prevented even if the cron retries) — review and send manually",
  ...
  settings: {
    ...
    content:
      "The Settings page has four collapsible sections: Athena Preferences (AI assistant), Email Preferences (branding for all outgoing emails), Backups & Cron (automated database backups), and Support Plan Defaults (post-launch retainer rates). Click any section header to expand or collapse it. Settings autosave after a short delay — a green cloud icon confirms the save. A manual Save button is also available in each section.",
    tips: [
      "Click a section header to expand/collapse — the chevron rotates to indicate open/closed state",
      "Green cloud (CloudCheck) icon in a section header = autosave confirmed for that section",
      "Athena: edit system prompt, temperature, max tokens, model list, and OpenAI fallback toggle",
      "Email: logo gallery (upload up to 3 logos to Vercel Blob, select active with radio), size slider, company name, sign-off, tagline, booking URLs, greeting toggles, and auto-approval email toggle",
      "Backups: view snapshots by tier (daily/weekly/monthly), trigger manual backup, restore, and configure retention policy",
      "Support: set default monthly rate, included hours, overage rate, and annual free months",
      "All settings persist to the database — they survive Vercel deploys and cold starts; changes propagate to all server functions within 60 seconds",
  ...
  emailPreferences: {
    ...
    content:
      "Configure branding and behavior for all outgoing emails — outreach, invoices, client invites, password resets, and approval requests. Company Name sets the 'From' display name. Logo Gallery lets you upload up to 3 logos to Vercel Blob — select the active one with the radio button. Sign-Off sets the closing lines in outreach emails. Tagline appears in every email footer. Booking URLs (up to 2) provide the 'Book a Free Discovery Call' CTA button in outreach emails — toggle the active one. The greeting toggles control whether outreach emails include the lead's name and/or company. The 'Auto-send approval email' toggle controls automatic client notification when a project reaches Client Approval. All emails are rate-limited to stay under Resend's 5 requests/second limit.",
    tips: [
      "Logo Gallery: click the upload area to upload an image (max 2MB) — stored in Vercel Blob; select the active logo with the radio button under each thumbnail; X button removes a logo (must keep at least one)",
      "The logo size slider (30–300px) adjusts how large the logo renders in email headers",
      ...
      "Booking URLs: add up to 2 booking links (e.g., cal.com URLs) — the active one is used as the 'Book a Free Discovery Call' CTA button in outreach emails; toggle the radio to switch the active URL",
      ...
      "All email settings autosave and persist to the database — they survive deploys and propagate within 60 seconds",
  ...
  outreach: {
    ...
    content:
      "Compose and send branded outreach emails to leads. There are two workflows: (1) Outreach page — click 'New Outreach' to write a subject and body, optionally assign leads, or save as a reusable template draft; (2) Leads page — check leads, click Continue in the floating action bar, pick a template, confirm, and send. Both workflows use the same branded template with your active logo, sign-off, booking CTA, and footer.",
    tips: [
      "Save a draft with no leads selected on the Outreach page to create a reusable email template",
      ...
      "Failed emails show an amber retry button — click to retry",
      "Only leads with email addresses appear in the lead picker and bulk select checkbox",
      "Bulk send from Leads: select leads with checkboxes → floating bar shows count → Continue → pick template → Send to N leads → confirm",
      "After bulk send, a toast shows: sent · skipped (no email) · failed counts — failed emails are marked FAILED and cannot accidentally be re-sent as duplicates",
      "Green mail icon on the Leads table = outreach sent; amber = draft pending — hover for the sent date",
      "The booking CTA button in outreach emails uses the active booking URL from Email Preferences",
    ],
  },
  adminGuide: {
    ...
    tips: [
      ...
      "HOW TO MANAGE CLIENTS: Pencil icon edits company, contact name, email, phone, website, address, sector, and notes. Trash icon archives the client (soft-delete) — the client disappears from the active list but invoices and financial history are preserved (confirm dialog warns). Click the company name to open the client detail page with projects and invoices. The Card checkbox tracks postcards sent.",
      ...
      "HOW TO SEND AN INVOICE: Click the blue mail icon on any DRAFT invoice → sends branded email to client with amount, due date, and 'View & Pay Invoice' button. The button disables while sending to prevent duplicate emails. Invoice status changes to SENT only after confirmed email delivery — if the email fails, the invoice stays DRAFT and you'll see an error toast. Payment structure: 40% at start, 30% mid-development, 30% at completion.",
      ...
      "HOW TO ISSUE A REFUND: Click the amber undo icon on a PAID invoice, or go to dashboard.stripe.com → Payments → find the payment → click 'Refund'. Full refunds automatically mark the invoice as REFUNDED via webhook + log activity. Partial refunds (e.g., refunding $50 of a $1,000 invoice via Stripe Dashboard) log a warning activity with the partial amount but do NOT change the invoice status — the invoice stays PAID so you can track the remaining balance. Client sees REFUNDED status in portal only for full refunds. Money returns to their card in 2-10 business days.",
      ...
      "HOW TO CONFIGURE SETTINGS: Settings has 4 collapsible sections — click headers to expand. Settings autosave after a short delay (green cloud icon confirms). Manual Save button available in each section. Athena: system prompt, temperature, max tokens, model list, OpenAI fallback toggle. Email: logo gallery (upload up to 3 logos to Vercel Blob — active logo selected via radio button), size slider (30–300px), company name, sign-off, tagline, booking URLs (up to 2 — toggle active one for outreach CTA button), greeting toggles, auto-approval email toggle. Backups: manual backup, restore, retention policy (daily/weekly/monthly counts), backup list with download/restore. Support Plan Defaults: monthly rate, included hours, overage rate, annual free months.",
