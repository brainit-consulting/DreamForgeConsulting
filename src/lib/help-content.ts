...
export const helpContent: Record<string, HelpSection> = {
  ...
  adminGuide: {
    ...
    tips: [
      ...
      "HOW TO BULK SEND FROM LEADS: Go to Leads → filter by sector or search → check the leads you want (leads without email are greyed out). A floating bar appears at the bottom: '[N] leads selected [Clear] [Send Outreach]'. Click Send Outreach → pick a template from the list → confirm → emails are sent sequentially with rate limiting. Toast shows sent/skipped/failed counts. This is the leads-first workflow — use it when you're already working the leads table.",
